...
tDFUI.HookScript = function(f, script, func)
  ...
  -- simply can't be hooked, and that shouldn't take the caller down with it.
  --
  -- Never attach this as a `frame.HookScript` method. Addons test for that
  -- method to detect a modern client and will then pass handlers that expect
  -- `self`, which vanilla script handlers never receive.
