...
module.enable = function(self)
        ...
        for i = 1, 12 do
            ...
            if frame then
                ...
                for i = 1, MAX_CONTAINER_ITEMS do
                    ...
                    local link = button and GetContainerItemLink(id, button:GetID())
            ...
            if frame then
                ...
                for i = 1, MAX_CONTAINER_ITEMS do
                    ...
                    if button and button:IsShown() and link then
                        -- "(.+)" is greedy and swallowed the second |h of the
                        -- link, so GetItemInfo got "item:123:0:0:0|h[Name]"
                        -- and returned nil, which made strlower() error out
                        -- on the first keystroke.
                        local _, _, istring  = string.find(link, "|H(.-)|h")
                        local name = istring and GetItemInfo(istring)
                        if name and strfind(strlower(name), strlower(string.gsub(this:GetText(), "([^%w])", "%%%1"))) then
                            button:SetAlpha(1)
                        end
                    end
                end
    ...
    -- the tooltip promises Esc works, but it was never wired up
