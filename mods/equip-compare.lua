...
module.enable = function(self)
    ...
    -- Call the helper directly rather than attaching it to the frames as a
    -- HookScript method. Other addons use "does this tooltip have a
    -- HookScript method?" to decide whether they're on a client with the
    -- modern tooltip API -- Tmog does exactly that -- and installing one here
    -- made them take a code path vanilla can't support. Their handlers expect
    -- `self` as an argument, which vanilla script handlers never pass.
    tDFUI.HookScript(GameTooltip, "OnShow", tDF.eqcompare.GameTooltipShow)
    tDFUI.HookScript(ShoppingTooltip1, "OnShow", tDF.eqcompare.ShoppingTooltipShow)
    tDFUI.HookScript(ShoppingTooltip2, "OnShow", tDF.eqcompare.ShoppingTooltipShow)

    if AtlasLootTooltip then
        tDFUI.HookScript(AtlasLootTooltip, "OnShow", tDF.eqcompare.GameTooltipShow)
        tDFUI.HookScript(AtlasLootTooltip2, "OnShow", tDF.eqcompare.GameTooltipShow)
    end
