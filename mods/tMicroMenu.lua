...
module.enable = function(self)
  ...
  local microbar_buttons =
  {
    ...
    },
    {
      ...
      f_OnClick = function(self, button, down)
        if ShopFrame_Toggle then ShopFrame_Toggle() end
      end
    },
    {
      Name = "mbLFT",
      ...
      f_OnClick = function(self, button, down)
        if LFT_Toggle then LFT_Toggle() end
      end
    },
    {
      Name = "mbEBC",
      ...
      f_OnEnter = function()
        GameTooltip:SetOwner(this, "ANCHOR_RIGHT")
        GameTooltip:SetText("Radio", 1, 1, 1, 1, true)
        GameTooltip:AddLine("Pick a radio station to listen to while you play.", nil, nil, nil, true)
        GameTooltip:Show()
      end,
      f_OnLeave = function()
        GameTooltip:Hide()
  ...
  -- only now that the replacement bar exists is it safe to hide blizzard's
  tDF_HideDefaultMicroButtons()
end